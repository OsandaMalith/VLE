...
<html>
    ...
    <body>
        ...
        <sql:query dataSource="${dbdriver}" var="TeacherUser">
            SELECT USERNAME FROM OSANDA.TEACHER where USERNAME= ?
        ...
        <c:choose>
            ...
            <c:otherwise>
                <sql:update dataSource="${dbdriver}" var="updateVar">
                    INSERT INTO OSANDA.COURSETEACHER (USERNAME, EKEY) 
                    VALUES (?, ?)
                    <sql:param value="${param.username}" />
                    <sql:param value="${param.code}" />
                </sql:update>
                ...
                <c:if test="${not empty param.unenroll}">
                    <sql:update dataSource="${dbdriver}" var="updateVar">
                        DELETE FROM OSANDA.COURSETEACHER WHERE USERNAME = ? AND EKEY = ?
                        <sql:param value="${param.username}" />
                        <sql:param value="${param.code}" />
                    </sql:update>
                    <c:if test="${updateVar>=1}">
